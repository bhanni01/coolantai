...
class TestCrossCheck:
    def test_estimate_within_tolerance_is_validated(self):
        ...
        checked = cross_check_estimates(
            make_candidate(),
            [estimate(PropertyName.THERMAL_CONDUCTIVITY, 0.15, "W/m·K")],
            [ester_profile(tc=0.14)],
        )
    ...
    def test_estimate_far_off_is_conflict(self):
        checked = cross_check_estimates(
            make_candidate(),
            [estimate(PropertyName.THERMAL_CONDUCTIVITY, 0.30, "W/m·K")],
            [ester_profile(tc=0.14)],
        )
    ...
    def test_temperature_property_uses_absolute_tolerance(self):
        ...
        checked = cross_check_estimates(
            make_candidate(),
            [estimate(PropertyName.FLASH_POINT, 250, "°C")],
            [ester_profile()],
        )
        assert checked[0].reference_check == "validated"
    ...
    def test_low_confidence_reference_is_ignored(self):
        checked = cross_check_estimates(
            make_candidate(),
            [estimate(PropertyName.THERMAL_CONDUCTIVITY, 0.15, "W/m·K")],
            [ester_profile(tc=0.14, tc_confidence=0.3)],
        )
        assert checked[0].reference_check is None
