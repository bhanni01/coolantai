...
is captured directly — the LLM never writes a ComplianceFlag itself. Code
covers any candidate/check the LLM skipped, and spec-exclusion screening
always runs in code (it is not an LLM decision). Writes only the
compliance_flags channel (disjoint from property_estimator's).
"""
...
from coolant_copilot.state import ComplianceFlag, GraphState
...
def make_compliance_checker_node(
    ...
    def compliance_checker(state: GraphState) -> dict:
        ...
        def check_pfas_definition(candidate_id: str) -> str:
            ...
            pfas_flags[candidate_id] = run_pfas_check(candidate, state.target_spec)
            return _flags_summary(pfas_flags[candidate_id])
        ...
        def check_material_compatibility(
            ...
            if candidate is None:
                return f"Unknown candidate_id '{candidate_id}'. Valid ids: {sorted(by_id)}"
            material_flags[candidate_id] = run_material_check(candidate, seal_materials)
            return _flags_summary(material_flags[candidate_id])

        candidate_lines = "\n".join(
            f"- {c.id}: "
            + ", ".join(f"{x.name} ({x.role}, CAS {x.cas_number or 'none'})" for x in c.components)
            for c in new
        )
        user_prompt = (
        ...
        run_tool_loop(
            ...
            SYSTEM_PROMPT,
            user_prompt,
            max_tool_calls=TOOL_BUDGET_PER_CANDIDATE * len(new),
        )
        ...
        return {"compliance_flags": state.compliance_flags + new_flags}
